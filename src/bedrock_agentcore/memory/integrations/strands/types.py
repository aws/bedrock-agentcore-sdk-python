...
"""Callback that derives one scalar metadata bag from a Strands message."""
...
class AgentCoreExactNamespaceStoreConfig(_AgentCoreMemoryStoreOptions):
    """Store configuration for exact-prefix namespace retrieval."""
...
class AgentCoreSubtreeStoreConfig(_AgentCoreMemoryStoreOptions):
    """Store configuration for subtree namespace-path retrieval."""
...
"""Configuration for one exact-namespace or subtree :class:`AgentCoreMemoryStore`."""
...
def resolve_namespace(template: str, actor_id: str, session_id: str) -> str:
    ...
    This deliberately mirrors the source runtime's ordered chained substitutions.
    Consequently, a ``{sessionId}`` token introduced by actor substitution is
    processed by the second pass.

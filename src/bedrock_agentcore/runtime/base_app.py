"""Base application class for Bedrock AgentCore runtime.

Provides common functionality shared between HTTP and A2A protocol implementations.
"""
...
class _BaseRequestContextFormatter(logging.Formatter):
    """Base log formatter including request and session IDs.

    Subclasses can provide extra_fields to include in every log entry
    (e.g., {"protocol": "A2A"}).
    """
...
class _BaseAgentCoreApp(Starlette):
    """Base class for Bedrock AgentCore applications.

    Provides shared functionality for HTTP and A2A protocol implementations:
    - Handler registration (entrypoint, ping, async_task decorators)
    - Ping/health check management
    ...
    def __init__(
        ...
        logger_name: str = "bedrock_agentcore.base_app",
    ...
    def entrypoint(self, func: Callable) -> Callable:
        ...
        func.run = lambda port=default_port, host=None: self.run(port, host)
        return func
    ...
    def _safe_serialize_to_json_string(self, obj):
        """Safely serialize object to JSON string with progressive fallback.
    ...
    def _convert_to_sse(self, obj) -> bytes:
        """Convert object to Server-Sent Events format.
    ...
    def run(self, port: Optional[int] = None, host: Optional[str] = None, **kwargs):
        ...
        Args:
            port: Port to serve on (defaults to protocol-specific port)

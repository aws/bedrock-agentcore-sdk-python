...
@requires_access_token(
    ...
    scopes=["default"],  # Changed from [] to ["default"] to pass validation
    auth_flow="M2M",
)
...
async def need_aws_jwt_token_async(*, access_token: str):
    """Test AWS JWT token retrieval with async function."""
    print(f"received AWS JWT token for async func: {access_token[:50]}...")
...
def need_aws_jwt_token_sync(*, access_token: str):
    """Test AWS JWT token retrieval with sync function."""
    print(f"received AWS JWT token for sync func: {access_token[:50]}...")
...
async def need_aws_jwt_token_rs256(*, access_token: str):
    ...
    print(f"received AWS JWT token (RS256) for async func: {access_token[:50]}...")
...
async def need_aws_jwt_token_with_tags(*, access_token: str):
    ...
    print(f"received AWS JWT token with tags: {access_token[:50]}...")
...
async def need_aws_jwt_custom_param(*, jwt_token: str):
    """Test AWS JWT token with custom parameter name."""
    print(f"received AWS JWT token in custom param: {jwt_token[:50]}...")

...
def semantic_memory() -> dict[str, str]:
    ...
    memory_id = os.environ.get("MEMORY_STORE_TEST_ID") or os.environ.get("MEMORY_PREPOPULATED_ID")
    if not memory_id:
        pytest.skip("MEMORY_STORE_TEST_ID or MEMORY_PREPOPULATED_ID is required for native Strands memory-store tests")
    return {"id": memory_id}

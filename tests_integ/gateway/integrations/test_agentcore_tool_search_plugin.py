...
class TestAgentCoreToolSearchPluginIntegration:
    ...
    def test_plugin_with_default_intent_provider(self):
        """Plugin initializes correctly with DefaultIntentProvider."""
        mcp_client = self._make_mcp_client()
        plugin = AgentCoreToolSearchPlugin(mcp_client=mcp_client)
        assert isinstance(plugin._intent_provider, DefaultIntentProvider)
...
@pytest.mark.integration
class TestDefaultIntentProviderIntegration:
    """Integration tests for DefaultIntentProvider with a real LLM."""
    ...
    def test_derive_intent_from_messages(self):
        """DefaultIntentProvider produces a non-empty intent string from messages."""
        provider = DefaultIntentProvider(message_window=3)
        messages = [
    ...
    def test_derive_intent_empty_messages(self):
        """DefaultIntentProvider returns empty string for empty messages."""
        provider = DefaultIntentProvider()
        intent = provider.derive_intent([])
        assert intent == ""

    def test_derive_intent_with_custom_model(self):
        """DefaultIntentProvider works with an explicitly provided model."""
        ...
        provider = DefaultIntentProvider(model=model)
        messages = [
    ...
    def test_derive_intent_respects_message_window(self):
        """DefaultIntentProvider only considers the last N messages."""
        provider = DefaultIntentProvider(message_window=2)
        messages = [

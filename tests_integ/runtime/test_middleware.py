...
""")
    ...
    proc = subprocess.Popen(
        ["python", str(agent_file)], cwd=str(tmp_dir), stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
def test_unauthenticated_request(middleware_server):
    ...
    response = requests.post(f"{middleware_server}/invocations", json={"message": "This should fail"})
    assert response.status_code == 200
    data = response.json()
...
def test_rate_limiting(middleware_server):
    ...
    response = requests.post(f"{middleware_server}/invocations", json={"message": "Request 1"}, headers=headers)
    data = response.json()
    assert data["success"] is True
    ...
    for i in range(first_remaining):
        response = requests.post(
            f"{middleware_server}/invocations", json={"message": f"Request {i + 2}"}, headers=headers
        )
        data = response.json()
        assert data["success"] is True

    # Next request should be rate limited
    response = requests.post(f"{middleware_server}/invocations", json={"message": "Rate limited"}, headers=headers)
    data = response.json()
    assert "error" in data

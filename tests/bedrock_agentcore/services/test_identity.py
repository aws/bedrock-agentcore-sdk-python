...
class TestIdentityClientAwsJwt:
    """Test IdentityClient AWS JWT functionality."""

    @pytest.mark.asyncio
    async def test_get_aws_jwt_token_success(self):
        """Test successful AWS JWT token retrieval."""
        region = "us-west-2"

        with patch("boto3.client") as mock_boto_client:
            # Mock for IdentityClient initialization
            mock_cp_client = Mock()
            mock_identity_client = Mock()
            mock_dp_client = Mock()
            mock_sts_client = Mock()

            # First 3 calls are for IdentityClient init, 4th is for STS
            mock_boto_client.side_effect = [mock_cp_client, mock_identity_client, mock_dp_client, mock_sts_client]

            identity_client = IdentityClient(region)

            # Mock STS response
            expected_token = "eyJhbGciOiJFUzM4NCIsInR5cCI6IkpXVCJ9.test-payload.test-signature"
            expected_expiration = datetime(2024, 1, 1, 0, 5, 0)
            mock_sts_client.get_web_identity_token.return_value = {
                "WebIdentityToken": expected_token,
                "Expiration": expected_expiration,
            }

            result = await identity_client.get_aws_jwt_token(
            ...
            assert result["expiration"] == expected_expiration
    ...
    @pytest.mark.asyncio
    async def test_get_aws_jwt_token_with_tags(self):
        """Test AWS JWT token retrieval with custom tags."""
        region = "us-west-2"
        ...
        with patch("boto3.client") as mock_boto_client:
            ...
            expected_token = "tagged-jwt-token"
            expected_expiration = datetime(2024, 1, 1, 0, 5, 0)
            mock_sts_client.get_web_identity_token.return_value = {
                "WebIdentityToken": expected_token,
                "Expiration": expected_expiration,
    ...
    @pytest.mark.asyncio
    async def test_get_aws_jwt_token_with_rs256(self):
        """Test AWS JWT token retrieval with RS256 algorithm."""
        region = "us-west-2"
        ...
        with patch("boto3.client") as mock_boto_client:
            ...
            expected_token = "rs256-jwt-token"
            mock_sts_client.get_web_identity_token.return_value = {
                "WebIdentityToken": expected_token,
                "Expiration": datetime(2024, 1, 1, 0, 5, 0),
            }
            ...
            call_args = mock_sts_client.get_web_identity_token.call_args[1]
            assert call_args["SigningAlgorithm"] == "RS256"

    @pytest.mark.asyncio
    async def test_get_aws_jwt_token_with_custom_duration(self):
        """Test AWS JWT token retrieval with custom duration."""
        region = "us-west-2"
        ...
        with patch("boto3.client") as mock_boto_client:
            ...
            expected_token = "long-lived-jwt-token"
            mock_sts_client.get_web_identity_token.return_value = {
                "WebIdentityToken": expected_token,
                "Expiration": datetime(2024, 1, 1, 1, 0, 0),
            }
    ...
    @pytest.mark.asyncio
    async def test_get_aws_jwt_token_with_multiple_audiences(self):
        """Test AWS JWT token retrieval with multiple audiences."""
        region = "us-west-2"
        ...
        with patch("boto3.client") as mock_boto_client:
            ...
            expected_token = "multi-audience-jwt-token"
            mock_sts_client.get_web_identity_token.return_value = {
                "WebIdentityToken": expected_token,
                "Expiration": datetime(2024, 1, 1, 0, 5, 0),
            }
            ...
            result = await identity_client.get_aws_jwt_token(audience=audiences)
    ...
    @pytest.mark.asyncio
    async def test_get_aws_jwt_token_failure(self):
        """Test AWS JWT token retrieval error handling."""
        region = "us-west-2"
        ...
        with patch("boto3.client") as mock_boto_client:
            ...
            # Mock STS error
            mock_sts_client.get_web_identity_token.side_effect = Exception("STS error: Access Denied")

            with pytest.raises(Exception) as exc_info:
                await identity_client.get_aws_jwt_token(audience=["https://api.example.com"])

            assert "STS error" in str(exc_info.value)

    def test_get_aws_jwt_token_sync_success(self):
        """Test synchronous AWS JWT token retrieval."""
        region = "us-west-2"
        ...
        with patch("boto3.client") as mock_boto_client:
            ...
            result = identity_client.get_aws_jwt_token_sync(
                audience=["https://api.example.com"],
                signing_algorithm="ES384",
                duration_seconds=300,
            )
    ...
    def test_get_aws_jwt_token_sync_with_tags(self):
        """Test synchronous AWS JWT token retrieval with tags."""
        region = "us-west-2"
        ...
        with patch("boto3.client") as mock_boto_client:
            ...
            expected_token = "sync-tagged-jwt-token"
            mock_sts_client.get_web_identity_token.return_value = {
                "WebIdentityToken": expected_token,
                "Expiration": datetime(2024, 1, 1, 0, 5, 0),
            }
            ...
            call_args = mock_sts_client.get_web_identity_token.call_args[1]
            assert call_args["Tags"] == custom_tags

    def test_get_aws_jwt_token_sync_failure(self):
        """Test synchronous AWS JWT token retrieval error handling."""
        region = "us-west-2"
        ...
        with patch("boto3.client") as mock_boto_client:
            ...
            mock_sts_client.get_web_identity_token.side_effect = Exception("STS sync error")

            with pytest.raises(Exception) as exc_info:
                identity_client.get_aws_jwt_token_sync(audience=["https://api.example.com"])

            assert "STS sync error" in str(exc_info.value)

...
@pytest.mark.parametrize(("want", "expected_top_k"), [(100, 100), (101, 100), (200, 100)])
async def test_result_cap_never_exceeds_service_top_k_limit(want: int, expected_top_k: int) -> None:
    """Never send a topK above AgentCore's limit, with or without a score floor.

    Without the clamp on this path, 101 and 200 reached the service verbatim and failed with a
    raw ``ValidationException`` while the same values succeeded once ``min_score`` was set.
    """
    client = client_returning([])
    await make_store(client, max_search_results=want).search("q")
    assert client.retrieve_memory_records.call_args.kwargs["searchCriteria"]["topK"] == expected_top_k

    floor_client = client_returning([])
    await make_store(floor_client, max_search_results=want, min_score=0.5).search("q")
    assert floor_client.retrieve_memory_records.call_args.kwargs["searchCriteria"]["topK"] == expected_top_k


@pytest.mark.parametrize("want", [101, 200])
async def test_call_time_result_cap_above_limit_is_clamped(want: int) -> None:
    """Clamp a per-call cap too: ``search_memory`` options reach the same wire field."""
    client = client_returning([])
    await make_store(client).search("q", {"max_search_results": want})
    assert client.retrieve_memory_records.call_args.kwargs["searchCriteria"]["topK"] == 100


async def test_result_cap_above_limit_warns_once(caplog: pytest.LogCaptureFixture) -> None:
    """Surface the reduced cap instead of silently under-delivering on every call."""
    client = client_returning([])
    store = make_store(client, max_search_results=200)
    with caplog.at_level("WARNING"):
        await store.search("q")
        await store.search("q")
    warnings = [record for record in caplog.records if "exceeds AgentCore's topK limit" in record.message]
    assert len(warnings) == 1


async def test_result_cap_at_limit_does_not_warn(caplog: pytest.LogCaptureFixture) -> None:
    """A cap at the service limit is a normal request, not a misconfiguration."""
    client = client_returning([])
    with caplog.at_level("WARNING"):
        await make_store(client, max_search_results=100).search("q")
    assert not [record for record in caplog.records if "topK limit" in record.message]
...
def vended_memory_client(data_plane: Mock) -> Any:
    ...
    with patch("boto3.Session", return_value=session):
        return MemoryClient(region_name="us-west-2")
...
async def test_store_accepts_the_sdk_memory_client_for_reads_and_writes() -> None:
    ...
    await store.search("q")
    assert data_plane.retrieve_memory_records.call_args.kwargs["searchCriteria"]["topK"] == 3
    await store.add_messages([user_message("hello")], AddMessagesContext(sequence_numbers=[0]))
    kwargs = data_plane.create_event.call_args.kwargs
    assert kwargs["memoryId"] == "mem-1"
    assert "clientToken" in kwargs

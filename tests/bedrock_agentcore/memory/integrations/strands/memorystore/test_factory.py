...
def test_assert_writable_topology_accepts_zero_or_one_writer() -> None:
    ...
    _assert_writable_topology([hand_built_store(), hand_built_store(name="prefs")])
    _assert_writable_topology([hand_built_store(writable=True), hand_built_store(name="prefs")])
...
def test_assert_writable_topology_rejects_multiple_writers() -> None:
    ...
    with pytest.raises(ValueError, match="at most one store may be writable"):
        _assert_writable_topology(
            [hand_built_store(name="a", writable=True), hand_built_store(name="b", writable=True)]
        )
...
def test_assert_writable_topology_can_require_writer() -> None:
    ...
    with pytest.raises(ValueError, match="no store is writable"):
        _assert_writable_topology([hand_built_store()], True)
    _assert_writable_topology([hand_built_store()], False)
...
def test_factory_accepts_the_sdk_memory_client_as_shared_client() -> None:
    ...
    stores = create_agentcore_memory_stores(**base_input(extraction=True, client=memory_client))
    assert {store._client for store in stores} == {data_plane}

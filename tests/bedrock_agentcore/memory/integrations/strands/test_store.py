...
from bedrock_agentcore.memory.integrations.strands.store import AgentCoreMemoryStore
from bedrock_agentcore.memory.integrations.strands.types import RESERVED_METADATA_PREFIX
...
def test_identity_matches_ecmascript_trim_boundaries(field: str) -> None:
    """U+0085 is identity content in JS, while a BOM-only identity is blank."""
    client = client_returning([])
...
def test_explicit_name_matches_ecmascript_trim_boundaries() -> None:
    """Retain U+0085 in a name and treat a BOM-only name as absent."""
    client = client_returning([])
    assert make_store(client, name=" \u0085 ").name == "\u0085"
    assert make_store(client, name="\ufeff").name == "strategy-s-actor-preferences"


def test_read_target_matches_ecmascript_trim_boundaries() -> None:
    """Retain U+0085 as a namespace and reject a BOM-only read target."""
    client = client_returning([])
...
def test_client_region_prefers_explicit_session_without_loading_invalid_ambient_profile(
    ...
    from bedrock_agentcore.memory.integrations.strands import store as store_module

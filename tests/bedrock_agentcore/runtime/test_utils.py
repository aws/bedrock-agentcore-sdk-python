...
class TestConvertComplexObjects:
    ...
    def test_nested_pydantic_models(self):
        ...
        person = Person(
            name="Alice", address=Address(street="123 Main St", city="Anytown")
        )
    ...
    def test_nested_sets_with_complex_objects(self):
        """Test sets containing complex objects."""
        ...
        test_set = {
            ItemModel(name="item1", value=10),
            ItemModel(name="item2", value=20),
        }
        result = convert_complex_objects(test_set)

        assert isinstance(result, list)
        assert len(result) == 2
        # Check that all items are converted to dicts
        for item in result:
    ...
    def test_mixed_complex_structures(self):
        ...
        post = PostModel(
            title="Hello World", content="This is a test post", author=user
        )

        complex_structure = {
            "users": [user, user],  # List of Pydantic models
            "profiles": {profile},  # Set of dataclasses
        ...
        # Verify profiles set (converted to list)
    ...
    def test_edge_cases(self):
        ...
        # This is a basic test - the function should handle this gracefully
        circular_list = []
        circular_list.append(circular_list)
        # The function should handle this without crashing
        try:
            result = convert_complex_objects(circular_list)
            # Result might be truncated or contain reference info
            assert isinstance(result, list)
        except RecursionError:
            # It's acceptable for the function to raise RecursionError for circular refs
            pass
    ...
    def test_pydantic_model_with_nested_dataclass(self):
        ...
        person = Person(
            name="Bob", address=Address(street="456 Oak St", city="Somewhere")
        )
        result = convert_complex_objects(person)
